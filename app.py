...
# runs in background thread to update VLM observation every SCAN_INTERVAL seconds
def background_scanner():
    global latest_observation, latest_elapsed, scanning
    while True:
        time.sleep(20)
        scanning = True
        result, elapsed = vision.analyze_frame(camera)  # unpack tuple
        with scan_lock:
...
warmup_thread = threading.Thread(target=vision.warmup, daemon=True)
warmup_thread.start()
...
<html>
<head>
  <title>Autonomous Inspection Robot</title>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #obs-timing { font-size: 11px; color: #555; }
    #scan-btn {
      align-self: flex-end;
      background: #1a2a1a;
...
<body>
  <h1>Autonomous Inspection Robot</h1>
  ...
  <div id="obs-panel">
    <div id="obs-header">
      ...
      <span id="obs-status">auto-scan every {{scan_interval}}s</span>
    </div>
    <div id="obs-text">Waiting for first scan...</div>
    <div id="obs-timing"></div> 
  ...
  <script>
    ...
    setInterval(() => {
      fetch('/observation').then(r => r.json()).then(data => {
        ...
        document.getElementById('obs-timing').textContent = data.elapsed ? `⏱ ${data.elapsed.toFixed(3)}s` : '';
        ...
        if (data.scanning) {
          ...
          statusEl.textContent = `auto-scan every ${data.interval}s`;
          statusEl.className = '';
    ...
    function manualScan() {
      ...
      fetch('/scan').then(r => r.json()).then(data => {
        document.getElementById('obs-text').textContent = data.text;
        document.getElementById('obs-status').textContent = `auto-scan every ${data.interval}s`;
        document.getElementById('obs-timing').textContent = data.elapsed ? `⏱ ${data.elapsed.toFixed(3)}s` : '';
...
@app.route('/')
def index():
    return render_template_string(DASHBOARD, scan_interval=SCAN_INTERVAL)
...
def observation():
    with scan_lock:
        return jsonify({"text": latest_observation, "scanning": scanning, "interval": SCAN_INTERVAL, "elapsed": latest_elapsed})

@app.route('/scan')
def manual_scan():
    global latest_observation, latest_elapsed, scanning
    scanning = True
    result, elapsed = vision.analyze_frame(camera)
    with scan_lock:
        latest_observation = result
        latest_elapsed = elapsed
    vision.log_observation(result)
    scanning = False
    return jsonify({"text": result, "interval": SCAN_INTERVAL, "elapsed": elapsed})
